...
var load_product = async(req,res,next)=>{
    wareHouseSchema.find({quatity:{$ne:0}}, async (error,product)=>{
        if (product==null){
            res.redirect('/customer/pages/ctm_index'); 
        }
        else{
            await productSchema.find({manufacturer_id:product.supplier_id,_id:product.product_id},(err,docs)=>{
                ...
                res.render('/customer/pages/ctm_index',{products:productChunks,pagination: { page: page, limit:num_page},paginateHelper: user_load.createPagination});
